...
const answers: string[] = readFileSync(join(dataDir, "wordle-answers.txt"), "utf8")
    .split("\n")
    .map((w) => w.trim().toLowerCase())
    .filter((w) => w.length === 5);

const validGuesses: Set<string> = new Set(
    readFileSync(join(dataDir, "wordle-valid-guesses.txt"), "utf8")
        .split("\n")
        .map((w) => w.trim().toLowerCase())
        .filter((w) => w.length === 5)
);
...
function getWordOfDay(): { word: string; wordNumber: number; date: string } {
...
router.post("/guess", (req, res) => {
    ...
    // validGuesses is a superset of answers, so this check covers all valid words
...
type TileResult = "correct" | "misplaced" | "wrong";

function evaluateGuess(guess: string, answer: string): TileResult[] {
    const result: TileResult[] = Array(5).fill("wrong");
